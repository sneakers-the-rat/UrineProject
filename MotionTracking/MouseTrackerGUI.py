...
class MouseTracker:
    ...
    def setInfo(self):
        ...
        self.dataq.makeFiles() #data handler makes relevant mouse and session files!


    def startSession(self):
        '''
        Initialises and runs an OSC server with default address and port, opens channel for incoming messages.
        Also starts task for NIDAQ sniff data acquisition.
        '''
        ...
        if len(self.mouseID) == 0:
            ...
            self.server = osc_server.ThreadingOSCUDPServer(('127.0.0.1',9999),dispatch)
            self.server.daemon = True
            self.bonsaiserver = threading.Thread(target=self.server.serve_forever)

            ###NIDAQ Setup: USB-6009 interfacing to receive sniff information from NIDAQ
   
            #Begin NIDAQ Communication and Server at the Same Time
            ...
            self.ni_signal = threading.Thread(target = self.dataq.ni_handler)
            self.ni_signal.daemon = True
            
            #Begin NIDAQ and Bonsai Communication Server at the same time
            self.bonsaiserver.start()
            ni_data.start(); self.ni_signal.start()
            self.server_active = True
            self.session_start = time.time()
            self.dataq.session_start = self.session_start
            
            #Session Running PopUp Notification
            StartNotification = PopUp("NOTIFICATION","Session started! Writing to files...")
            StartNotification.show()

   #          #Collect Data from NIDAQ.. NANDATTE?? NANI KORE?
			# while self.server_active == True:
			# 	self.dataq.AI_Handler()
    ...
    def analyze(self):
        '''
        Draws and displays mouse path in THREE figures via matplotlib. Figure 1 is a heatmap,
        Figure 2 draws a line between the (x,y) coordinates, which superimpose another heatmap.
        Calculate sniff frequency from raw sniff data and make a heatmap of sniff frequency
        Also enters metadata (average overall mouse location AND average sniff frequency) to a file with all mouse information
        '''
        if self.server_active:
            ...
            #Does both location and sniff analysis if we are running sniff with this session
            if self.sniffing == "Y":
                generate_inhale_exhale(self.dataq.session_dir,self.dataq.analysis_dir)
